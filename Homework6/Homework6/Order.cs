...
namespace Homework6
{
    ...
    public class OrderDetails  //订单明细
    {
        ...
        {
            if (Name == ((OrderDetails)detail).Name)
                return true;
            else
                return false;
        }
    ...
    public class OrderService  //订单服务
    {
        ...
        //用订单金额范围查询订单
        public Order[] QueryOrderByTotalPrice(float minTotalPrice, float maxTotalPrice)
        {
            var orders = from o in orderList
                         where o.TotalPrice >= minTotalPrice && o.TotalPrice <= maxTotalPrice
                         orderby o.TotalPrice descending
                         select o;
            if (!orders.Any())
    ...
    public class Program
    {
        ...
        static void Show(OrderService.Info[] order)
        {
            Console.WriteLine("---------------------------------------------------------------");
